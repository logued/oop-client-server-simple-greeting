...
/**
 * A basic Client/Server program that demonstrates how a Server
 * creates a ServerSocket that allows a client to connect to it.
 * Once connected, and the Socket is established, the client sends
 * a greeting message to the server and the server responds by sending a reply to the client.
...
public class GreetingServer {
    ...
    final static int PORT_NUMBER = 8888;  // could be any port from 1024 to 49151 (that doesn't clash with other Apps)
    ...
    public void start() {
        ...
        try (ServerSocket serverSocket = new ServerSocket(PORT_NUMBER);
            ...
            String message = socketReader.readLine(); // read the incoming message - terminated by a newline ("\n")
            // program stops and waits here until input (followed by '\n') arrives.

            System.out.println("The GreetingServer has received the following message from a client: " + message);
            System.out.println("The server will now replying to the client.");

            if ("hello server".equals(message)) {
                socketWriter.println("hello client");    // send response back to client
            } else {
                socketWriter.println("unrecognised greeting");
            }
        ...
        // the server socket and the streams.
    }
